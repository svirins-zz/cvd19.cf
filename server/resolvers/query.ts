...
const resolvers = {
  async results(_parent, { countries, date }, { getResults }) {
    ...
    let formatted = countryNames
      .reduce((acc, countryName) => {
        ...
        const withCountryName = countryResults.map((result, index) => ({ ...result, gR: getGrowthRate(index, countryResults), country: { name: countryName } }))
  ...
  async countries(_parent, { names }, { getResults }) {
    ...
    let formatted = (names && names.length > 0 ? names : Object.keys(results))
      .reduce((acc, countryName) => {
        ...
        const updatedResults = countryResults.map((result, index) => ({ ...result, gR: getGrowthRate(index, countryResults) }))
        const mostRecentIndex = countryResults.length - 1
        const mostRecent = countryResults[mostRecentIndex]
        mostRecent.gR = getGrowthRate(mostRecentIndex, updatedResults)
  ...
  async country(_parent, { name }, { getResults }) {
    ...
    results = results.map((result, index) => ({
      ...result,
      gR: getGrowthRate(index, results)
    }))
